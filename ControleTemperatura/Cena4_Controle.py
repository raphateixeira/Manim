...
por baixo até a entrada negativa do comparador."""
...
class Cena4_Controle(Scene):
    def construct(self):
        ...
        t_arr = sim["t"]; r_arr = sim["r"]; T_arr = sim["T"]; u_arr = sim["u"]; e_arr = sim["e"]
        ...
        # NOTA: next_to()/move_to() alinham pela CAIXA DELIMITADORA de cada
        # bloco, não pelo terminal de ligação real. Como os rótulos (só em
        # cima, sem nada embaixo para compensar) deixam essa caixa
        # assimétrica, e o erro se acumula bloco a bloco ao longo da malha,
        # corrigimos a posição vertical de CADA bloco pelo seu terminal real
        # logo após posicioná-lo — assim toda a malha fica exatamente na
        # mesma altura (row_y) e cada ramo sai rigorosamente horizontal.
        ...
        forno = build_forno(temp_tracker, power_tracker, width=3.3, height=2.35)
        sensor = build_sensor(width=1.0, height=0.55)

        row_y = comparador.get_right_pt()[1]  # comparador é simétrico: referência de altura confiável
        ...
        forno.next_to(atuador, RIGHT, buff=0.85)
        forno.shift(UP * (row_y - forno.get_power_in()[1]))
        ...
        fio_at_forno = Line(atuador.get_out(), forno.get_power_in(),
        ...
        # realimentação: sai da planta (mesma altura row_y), desce, passa
        # pelo sensor (reposicionado pelo terminal real) e volta por baixo,
        # em nível único, até a entrada negativa do comparador.
        tap = forno.get_output_tap()
        ...
        self.play(LaggedStart(
            ...
            Create(fio_at_forno), FadeIn(forno, shift=LEFT * 0.2),
        ...
        explicacao = Text("O controlador transforma o erro e(t) em sinal de controle u(t) para o atuador.",
                           font_size=16, color=WHITE, t2c={"e(t)": RED_ERR, "u(t)": BLUE_CTRL})
        ...
        eixo_ry = signal_axes(x_range=[0, t_max, 4], y_range=[20, 95, 25], x_length=9.4, y_length=1.1)
        eixo_ry.move_to(np.array([0.1, -0.7, 0]))
        titulo_ry = Text("Referência r(t)  ×  Temperatura T(t)", font_size=14, color=WHITE)
        titulo_ry.next_to(eixo_ry, UP, buff=0.06)

        eixo_u = signal_axes(x_range=[0, t_max, 4], y_range=[0, 1.0, 0.5], x_length=9.4, y_length=0.95)
        eixo_u.next_to(eixo_ry, DOWN, buff=0.25)
        titulo_u = Text("Sinal de controle u(t)", font_size=14, color=BLUE_CTRL)
        titulo_u.next_to(eixo_u, UP, buff=0.05)

        eixo_e = signal_axes(x_range=[0, t_max, 4], y_range=[-10, 50, 20], x_length=9.4, y_length=0.95)
        eixo_e.next_to(eixo_u, DOWN, buff=0.25)
        titulo_e = Text("Erro e(t) = r(t) − T(t)", font_size=14, color=RED_ERR)
        ...
        curva_T = growing_curve(eixo_ry, t_arr, T_arr, t_tracker, color=HEAT, stroke_width=3)
        ...
        self.add(curva_r, curva_T, curva_u, curva_e)
        self.wait(0.2)
